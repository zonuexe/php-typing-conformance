...
That writes `conformance/results/results.html` and a page per test under
`conformance/results/tests/`, both git-ignored. GitHub Actions runs the same two
commands on every push to master and publishes the result to GitHub Pages, so
the published site is always rendered from the committed data rather than from
whatever HTML someone last regenerated by hand.
