...
`7.0.0-beta20`) run through the same adapter with a config of its own
...
psalm version cell names it on a second line — `6.17.1 / next:
7.0.0-beta20`), and the detail pages show it as a full row. Only the CLI is
...
in `psalm-next.xml`; and the version banner reads `Psalm 7.0.0-beta20@…`,
